...
interface AccountEventSubmissionTeam {
  id: string
  name: string
  slug: string
  role: 'member' | 'admin'
...
const props = defineProps<{
  ...
  team: AccountEventSubmissionTeam | null
...
const teamFormationAvailability = computed(() =>
  getTeamFormationAvailability(props.event, ownApplicationStatus.value, hasPersistedTeam.value)
)
...
const provisionalCurrentTeam = computed(() => {
  ...
  const provisionalTeamId = `provisional-team:${props.event.id}:${actor.value.platformUser.id}`
  return {
    ...
    members: [
      {
        id: `provisional-member:${props.event.id}:${actor.value.platformUser.id}`,
...
const submissionWorkspace = useTeamSubmissionWorkspace(
  computed(() => props.event),
  {
    visibleEventId: computed(() => props.event.id),
    team: displayedTeam,
    canViewSubmission,
...
const submissionUnavailableDescription = computed(() => {
  ...
  if (!displayedTeam.value) {
    return 'There is no active team submission available for your account in this event.'
  }

  return 'The current submission surface could not be resolved right now.'
})
const createTeamAvailability = computed(() =>
  getCreateTeamAvailability(props.event, ownApplicationStatus.value, hasPersistedTeam.value)
)
...
<template>
  ...
  >
    <AppAlert
      ...
      description="Checking what submission actions are available to you in this event."
    />

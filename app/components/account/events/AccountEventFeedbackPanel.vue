...
const summary = computed(() => summaryResponse.value?.data ?? {
  ...
  questionSummaries: [],
  comments: []
} satisfies EventFeedbackSummary)

const commentsCount = computed(() => summary.value.comments.length)
const showPreCompletionNotice = computed(() => props.eventState !== 'completed')
...
function getQuestionDistributionRows(question: EventFeedbackQuestionSummary) {
  return [
    ...[...eventFeedbackRatingValues].reverse().map(score => ({
      key: `rating-${score}`,
      label: String(score),
    ...
    {
      key: 'not-applicable',
      label: eventFeedbackNotApplicableLabel,
...
<template>
  <div class="space-y-6">
    <AppAlert
      ...
      description="This tab will start collecting responses once the event reaches the completed state."
    />

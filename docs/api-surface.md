...
- D1 session bookmarks are carried only in the `X-D1-Bookmark` request and response header. They remain transport metadata and are not part of any domain payload contract. HTTP requests use strong consistency by default; only the named `getPublicReplicaDatabase` accessor may opt into an unconstrained session for source-allowlisted immutable/cacheable public reads, while mutations, actor, consent, permission, and read-after-write requests use primary or an incoming bookmark. Raw prepared statements and batches use the request session accessors, and HTTP handlers cannot inject database access through H3 context.
...
- Public event HTML, JSON, and versioned media responses have explicit cache freshness and validators. Browser responses use `Cache-Control: public, max-age=30, stale-if-error=0`; Cloudflare edge responses use `Cloudflare-CDN-Cache-Control: public, max-age=30, stale-if-error=0`. Public payloads do not use `s-maxage`, one-year immutable freshness, or stale-on-error behavior.
- Public cache keys include the request path, query, and content-negotiation dimensions. A cache hit can bypass the Worker, so D1 visibility or revision checks apply only on a miss or revalidation. Since the runtime does not own a globally authenticated purge mechanism, hide, image replacement/removal, public-gallery changes, and event deletion have a bounded visibility window of at most 30 seconds at the browser or Cloudflare edge.
- Public image responses are streamed through exact-current-revision URLs and bounded named Cloudflare Images variants with negotiated AVIF, WebP, or JPEG output. Private or mutable originals remain authorization-gated; invalid or stale public media versions return not found before R2 or Images access.

...
| Get public participation certificate | `GET /api/public/events/:slug/participants/:userId/certificate` | public or authenticated user | Returns the participation certificate for the exact public event slug and participant user id. Available only while the participant's application is approved with effective attendance (admin override first, otherwise the Luma check-in), the participant has not disabled certificate generation, and the account is active; otherwise responds not found. Includes the participant name, event identity and type, certificate date, location, the Hackathon submission track when one applies or the single configured Build track when unambiguous, the certificate ID, and the event display background image. For completed Hackathons it also includes the team name, project name, final placement, and won prize names from the competition outcome model; these stay empty before completion. |

...
- D1 session bookmarks are carried only in the `X-D1-Bookmark` request and response header. They remain transport metadata and are not part of any domain payload contract. Read-only requests may begin with an unconstrained session; mutations, strong reads, and read-after-write requests use primary or an incoming bookmark.

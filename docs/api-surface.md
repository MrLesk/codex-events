...
| Get public participation certificate | `GET /api/public/events/:slug/participants/:userId/certificate` | public or authenticated user | Returns the participation certificate for the exact public event slug and participant user id. Available only while the participant's application is approved with effective attendance (admin override first, otherwise the Luma check-in), the participant has not hidden the certificate, and the account is active; otherwise responds not found. Includes the participant name, event identity and type, certificate date, location, the Hackathon submission track when one applies or the single configured Build track when unambiguous, the certificate ID, and the event display background image. For completed Hackathons it also includes the team name, project name, final placement, and won prize names from the competition outcome model; these stay empty before completion. |
...
| Download participation certificate PDF | `GET /api/public/events/:slug/participants/:userId/certificate.pdf` | authenticated user | Renders the print-oriented certificate PDF as an attachment, including the public verification link and a QR code that resolves to the live certificate page. Follows the same availability rules as the certificate read. |
| Preview a participation certificate | `GET /api/public/events/:slug/participants/preview/certificate` (and `.png` / `.pdf`) | public or authenticated user | Renders a synthetic certificate for design review on the reserved `preview` participant id, driven by `name`, `type`, `rank`, `track`, `project`, `team`, and `prizes` query parameters with sample defaults. Requires only a publicly visible event slug and reads no application data. The preview PDF skips the authenticated-session requirement, preview image responses are never cached, and the preview page presents itself as a sample and is excluded from search indexing. |
...
| Set own certificate visibility | `POST /api/events/:eventId/applications/me/actions/set-certificate-visibility` | approved user | Hides or re-publishes the caller's own participation certificate. Allowed only for the caller's approved application; hidden certificates respond not found on every public certificate read. The action is audit logged. |

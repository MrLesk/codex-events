...
| List event applications | `GET /api/events/:eventId/applications` | staff, event admin, or platform admin | Returns paginated application records for participant-visibility and review workflows, including `selectedTrackId` for track chips and approved-participant track filters. Supports `page`, `page_size` up to `100`, and optional `status`; response metadata includes total count and status counts. Staff access is read-only. |

...
| Update submission | `PATCH /api/hackathons/:hackathonId/teams/:teamId/submission` | team admin | Allowed only in `submission_open`. When the hackathon has configured tracks, the body must keep a valid selected track. `summary`, `repositoryUrl`, and `demoUrl` are required only when the hackathon configuration enables those submission requirements. |
| Submit project | `POST /api/hackathons/:hackathonId/teams/:teamId/submission/actions/submit` | team admin | Allowed only in `submission_open`. When the hackathon has configured tracks, the submission must already reference exactly one valid track. Any submission fields configured as required by the hackathon must already be present and valid. |

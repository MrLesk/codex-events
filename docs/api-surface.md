...
| Get public participation certificate | `GET /api/public/events/:slug/participants/:userId/certificate` | public or authenticated user | Returns the participation certificate for the exact public event slug and participant user id. Available only while the participant's application is approved with a recorded check-in and the account is active; otherwise responds not found. Includes the participant name, event identity and type, certificate date, location, the Hackathon submission track when one applies, the certificate ID, and the event display background image. |

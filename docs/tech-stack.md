...
- Generated structured-operation output contracts are deterministic per-operation factories cached in the Worker isolate. A normal route resolves only its own output contract, while catalog loading preserves the same operation IDs, REST/MCP bindings, and input/output contracts. The shared executor is the sole owner of final operation-envelope output validation; page routes validate their page result before envelope construction.
...
- Strong HTTP requests start with `first-primary`, unless an incoming `X-D1-Bookmark` carries a genuine opaque bookmark that anchors the session. The HTTP boundary rejects only the exact documented D1 constraints `first-primary` and `first-unconstrained` before calling `withSession`; every other non-empty bookmark remains opaque. A request uses one consistency constraint and one shared Drizzle client for all domain work. Raw SQL uses the same request session's `prepare` and `batch` methods.

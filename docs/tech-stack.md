...
- The application database passed to domain code is created from a capability-narrowed request-session binding containing only `prepare` and `batch`. Its Drizzle `$client` does not expose the raw D1 binding or session construction; raw request-session `prepare` and `batch` access is available only through `getDatabaseSession(event)`.

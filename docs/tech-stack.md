...
- The capability-narrowed request-scoped D1 session adapter attributes every executed prepared statement and batch in the request timing system. `d1-exec-total` reports elapsed adapter time, execution count, statement count, and bounded overflow; `d1-db-total` reports the known sum of D1 `timings.sql_duration_ms` plus unknown metadata count, attempts, and serving-location summary. Up to eight `d1-exec-N` entries identify the request-local ordinal, `prepare` or `batch` API, execution kind, statement count, elapsed duration, known database duration, attempts, and bounded `served_by_region`, `served_by_colo`, and `served_by_primary` values. Missing result metadata is explicit; SQL text, bound values, results, identity data, table names, and raw binding/session capabilities are never emitted.

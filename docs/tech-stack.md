...
- The application database passed to domain code is created from a capability-narrowed request-session binding containing only `prepare` and `batch`. It does not expose a public Drizzle `$client`, raw D1 binding, or session construction; raw request-session `prepare` and `batch` access is available only through `getDatabaseSession(event)`.
...
- The managed public-media scope is event background and banner images, the platform default event background, and public event-gallery photo responses. These URLs are streamed from managed storage through Cloudflare Images and carry the exact current event or platform `media_revision`; callers do not request arbitrary dimensions.
- Public gallery photos expose bounded named variants: `preview` is a 720px `scale-down` transform and `original` selects a bounded full-display `scale-down` transform capped at 2400px. Both public variants are transformed WebP responses. Stored R2 originals remain private behind authorization and are never a public fallback.
- Event and platform-default media revisions are updated atomically with image, visibility, replacement, removal, and public-gallery mutations. `updated_at` is not an immutable media revision. Generated certificate PNGs and winner or published-project profile icons are outside this managed cache scope and remain private, `no-store` responses; `profileIconUpdatedAt` is not a public managed-media revision.
- Newly issued public event HTML, JSON, and versioned event, platform-default, or gallery media responses use `Cache-Control: public, max-age=30, stale-if-error=0` for browsers and `Cloudflare-CDN-Cache-Control: public, max-age=30, stale-if-error=0` for Cloudflare edge caching. They do not use `s-maxage`, one-year `immutable` freshness, or stale-on-error behavior.
- A Cloudflare cache hit can be served without invoking the Worker, and Cache API deletion is data-center local rather than a global purge. D1 validation therefore applies on a miss or revalidation, not to an already-served hit. The application does not own a global runtime purge credential; new managed URLs have the documented 30-second freshness window, not a strict global revocation guarantee.
- Public managed-media routes validate visibility, the exact current revision, and the managed object before reading R2 or invoking Images. Missing, invalid, or stale revisions return not found, and public callers never receive stored originals.
- Legacy public photo URLs issued before this managed-media contract may have `public, max-age=31536000, immutable` browser and edge freshness. A new Worker cannot revoke a browser or edge hit for those URLs, so they are not covered by the 30-second bound. A one-time operator purge by URL prefix or retirement of the legacy media namespace is required before migration is considered complete; see `OPERATOR.md`.

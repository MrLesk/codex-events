...
describe('Worker secret writer', () => {
  ...
  test('writes the secret-bulk JSON file and merges an optional Luma secret file', async () => {
    const directory = await mkdtemp(join(tmpdir(), 'codex-events-worker-secrets-'))

    try {
      const outputPath = join(directory, 'worker-secrets.json')
      const mergePath = join(directory, 'luma-secret.json')
      ...
      expect(payload.NUXT_LUMA_WEBHOOK_SECRET).toBe('webhook-secret')
    } finally {
  ...
  test('parses output and merge arguments', () => {
    expect(parseScriptArgs(['secrets.json', '--merge', 'luma.json'])).toEqual({

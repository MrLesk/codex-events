Feature: Participant team formation workspace
  ...
  Scenario: Regular user creates a team from the public hackathon entry surface
    Given the saved "regular_user" Auth0 session state exists
    When I open the participant application page for hackathon slug "participant-team-create-fixture-hackathon" with the saved "regular_user" session
    And I open the participant team workspace link

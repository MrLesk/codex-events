...
CREATE TABLE `__new_judge_assignments` (
	...
	CHECK (`pitch_score` is null or (`pitch_score` >= 1 and `pitch_score` <= 5)),
...
INSERT INTO `__new_judge_assignments`(
	...
	"status",
	CASE
		WHEN "pitch_score" is null THEN null
		WHEN "pitch_score" <= 1 THEN 1
		WHEN "pitch_score" <= 3 THEN 2
		WHEN "pitch_score" <= 5 THEN 3
		WHEN "pitch_score" <= 7 THEN 4
		ELSE 5
	END,
	"pitch_comment",
	"assigned_at",
...
--> statement-breakpoint
INSERT INTO `__new_judge_criterion_scores`(
	"id",
	"judge_assignment_id",

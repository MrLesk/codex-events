...
- The same single Nitro `beforeResponse` hook owns the fail-closed API cache policy. Every `/api/**` response must emit `Cache-Control: private, no-store` and `Cloudflare-CDN-Cache-Control: private, no-store` on success and error responses, including `/api/session` and returned `Response` bodies. Only the explicit public/versioned event and media path allowlist may retain the exact public 30-second directives; never add a handler-local shared-cache exception. Runtime/source tests must cover protected JSON and `Response` bodies, unauthenticated session isolation, public allowlist preservation, and real-browser rejection of protected `CF-Cache-Status: HIT` or `Age` evidence.

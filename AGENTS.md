...
- The application database passed to domain code must be created from a capability-narrowed request-session binding containing only `prepare` and `batch`. Never spread or pass the raw D1 binding into Drizzle: `AppDatabase.$client` must not expose `withSession`, `exec`, `dump`, or any other raw binding or session-construction capability. Raw request-session `prepare` and `batch` access belongs only to `getDatabaseSession(event)`.
- Every production module under `server/` that can be reached by HTTP must use the request-scoped database or session accessors. It must not import or call raw D1 binding resolution, create a standalone Drizzle client or database access object, use replica consistency, or read/write H3 context database fields or test injection APIs. Non-HTTP construction and injection live only in the explicit source-allowlisted infrastructure/test boundary and its module-private state.
...
- The TASK-432.6 managed public-media scope is event background and banner images, the platform default event background, and public event-gallery photo responses. Newly issued managed URLs carry the exact current event or platform `media_revision`; `variant=original` for a public gallery photo means a bounded full-display Images transform, never the stored R2 original. Generated certificate PNGs and winner or published-project profile icons are outside this managed cache scope and remain private, `no-store` responses; `profileIconUpdatedAt` is a request/version guard, not a managed public-media revision.
- Newly issued managed event, platform-default, and gallery responses use the documented 30-second browser and Cloudflare freshness window. A cache hit can bypass the Worker, D1 visibility/revision checks apply only on a miss or revalidation, and Cache API deletion is not a global purge. Do not claim strict revocation for a cached response or add a runtime purge secret.
- Legacy public photo URLs issued before the managed-media contract may remain browser- or edge-cacheable for their original immutable lifetime (up to one year) and cannot be revoked by new Worker validation. Before relying on the managed contract after a migration, an operator must purge the affected legacy URL prefixes or retire the old media namespace as documented in `OPERATOR.md`; this is a one-time edge operation, not an application request path.

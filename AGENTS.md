...
- Keep Cloudflare Workers cache disabled (`cache.enabled=false`) in the tracked Wrangler config and every generated deployment config. Zone Cache Rules must bypass `/api/**` or respect the origin no-store directives; no rule may use a Cookie-insensitive cache key for protected APIs. Public/versioned routes remain explicit origin-cache contracts. Release verification must use a safe, opt-in remote edge smoke test; local tests do not simulate a cache hit and no runtime purge is assumed.

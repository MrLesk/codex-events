...
- Strong consistency is the default for every HTTP database request. Only the named `getPublicReplicaDatabase` accessor may opt into replica consistency, and its use is source-allowlisted to vetted immutable/cacheable public reads; actor, consent, permission, lifecycle, mutation, and read-after-write paths remain strong.
...
- HTTP handlers under `server/api/` and `server/routes/` must use the request-scoped database or session accessors. They must not import or call raw D1 binding resolution, create a standalone Drizzle client or database access object, opt into replica consistency except through the source-allowlisted public accessor, or use H3 context database fields/`setDatabase`; direct database injection is limited to explicit non-HTTP test or infrastructure events held in module-private state.

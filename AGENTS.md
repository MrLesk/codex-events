...
- Protected declarative and imperative query reads disable transport-level retries and use the shared request owner. The owner retains every non-abort settlement, including transport failures and terminal business responses, until an explicit refresh/action invalidates it; a changed request key or authorization generation creates a genuinely new read. Aborts remain aborts and must not become retries.
